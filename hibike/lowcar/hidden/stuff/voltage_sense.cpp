...
voltage_sense::voltage_sense(eeprom& ee_prom, VoltageTracker& v_tracker) :
...
void voltage_sense::setup_sensing()
{
  ...
  if(sense_voltage_tracker.get_triple_calibration())
  {
    sense_eeprom.update_triple_calibration();  //and then update the calibration values I use based on what I now know
  }
  else
  {
    float val = sense_eeprom.get_calibration();
    if(val > 0) //by convention, val is -1 if there's no calibration.
    {
      sense_voltage_tracker.set_voltage(VREF_GUESS, val);
    }
  }
}
...
void voltage_sense::measure_cells() //measures the battery cells. Should call at least twice a second, but preferrably 5x a second.  No use faster.
{
    ...
    float calib_0 = sense_voltage_tracker.get_calib(0);
    float calib_1 = sense_voltage_tracker.get_calib(1);
    float calib_2 = sense_voltage_tracker.get_calib(2);

    float new_v_cell1 = float(r_cell1) * (R2 + R5) / R5 * calib_0 / ADC_COUNTS;
    float new_v_cell2 = float(r_cell2) * (R3 + R6) / R6 * calib_1 / ADC_COUNTS;
    float new_v_cell3 = float(r_cell3) * (R4 + R7) / R7 * calib_2 / ADC_COUNTS;

    sense_voltage_tracker.set_voltage(V_CELL1, new_v_cell1);
    sense_voltage_tracker.set_voltage(V_CELL2, new_v_cell2);
    sense_voltage_tracker.set_voltage(V_CELL3, new_v_cell3);

    sense_voltage_tracker.set_voltage(DV_CELL2, new_v_cell2 - new_v_cell1);
    sense_voltage_tracker.set_voltage(DV_CELL3, new_v_cell3 - new_v_cell2);

    sense_voltage_tracker.set_voltage(V_BATT, new_v_cell3);
}

uint8_t voltage_sense::handle_calibration() //called very frequently by loop.
{
  if (digitalRead(CALIB_BUTTON)) //I pressed the button, start calibrating.
    ...
    if(sense_voltage_tracker.get_triple_calibration() && calib_0 == ADC_REF_NOM && calib_1 == ADC_REF_NOM & calib_2 == ADC_REF_NOM)
    {
      ...
      calib_0 = sense_voltage_tracker.get_calib(0);
      calib_1 = sense_voltage_tracker.get_calib(1);
      calib_2 = sense_voltage_tracker.get_calib(2);
      sense_eeprom.write_triple_eeprom(calib_0, calib_1, calib_2);
      return NEW_CALIB;
    }
    else if(!sense_voltage_tracker.get_triple_calibration() && sense_eeprom.get_calibration() == -1.0) //i'm not in triple calibration mode, and i haven't been calibrated.
    {
      float vref_new = calibrate();
      sense_eeprom.write_single_eeprom(vref_new);
      return NEW_CALIB;
    }
    else //I have already been calibrated with something.  reset it.
    {
      ...
      sense_voltage_tracker.set_voltage(VREF_GUESS, ADC_REF_NOM);
      sense_voltage_tracker.set_calib(0, ADC_REF_NOM);
      sense_voltage_tracker.set_calib(1, ADC_REF_NOM);
      sense_voltage_tracker.set_calib(2, ADC_REF_NOM);

      sense_eeprom.clear_eeprom();
      return CLEAR_CALIB;
    }
  }
}
...
//it then calculates its best guess for vref.
{
    ...
    sense_voltage_tracker.set_calib(0, vref1);
    sense_voltage_tracker.set_calib(1, vref2);
    sense_voltage_tracker.set_calib(2, vref3);

    //avg all vrefs together to get best guess value
    float vref_guess = (vref1 + vref2 + vref3) / 3.0f;
    sense_voltage_tracker.set_voltage(VREF_GUESS, vref_guess);
    return vref_guess;
}
