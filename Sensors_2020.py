...
ratbox_port = "/dev/ttyACM0" # change to correct port
...
def identify_relevant_ports(working_ports):
    ...
    def maybe_identify_sensor(serial_port, timeout, msg_q):
        ...
        try:
            ...
            if is_ratbox(msg):
                #should be rb for ratbox
                object_type = msg[0:2]
                #r for referee
                object_alliance = "r"#msg[2:3]
                #msg_q.put((object_type, object_alliance, serial_port))
                msg_q.put((object_type, serial_port))
...
def is_ratbox(sensor_msg):
    """Check whether a message is sent from the ratbox"""
    return sensor_msg[0:2] == "rb"
...
def recv_from_bid(ser, alliance_enum):
    ...
    while True:
        ...
        if payload_list[0] == "rs": #rs=ratstatus
            send_dictionary = {"blue": payload_list[1], "gold": payload_list[2], "king": payload_list[3]}
            ## TODO: fix lcm lines later
...
def main():
    ...
    recv_thread_rat = threading.Thread(
        target=recv_from_bid, name="receive thread rat",
        args=([rat_serial_ref, ALLIANCE_COLOR.BLUE]))
    send_thread_rat = threading.Thread(
        target=send_to_bid, name="send thread rat",
        args=([rat_serial_ref, ALLIANCE_COLOR.BLUE]))

    recv_thread_rat.daemon = True
    send_thread_rat.daemon = True

    recv_thread_rat.start()
    send_thread_rat.start()
